...
UserInterface::UserInterface(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
	:CGameObject(1)
{
	CTexturedRectMesh* pMesh = new CTexturedRectMesh(pd3dDevice, pd3dCommandList, 100.f, 100.f, 0.f, 0.f, 0.f, 0.f);
	SetMesh(pMesh);

	CreateShaderVariables(pd3dDevice, pd3dCommandList);

	CTexture* UITexture = new CTexture(1, RESOURCE_TEXTURE2D, 0);
	UITexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Model/Textures/red.dds", 0);

	UserInterfaceShader* UIShader = new UserInterfaceShader();
	UIShader->CreateShader(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature);
	UIShader->CreateShaderVariables(pd3dDevice, pd3dCommandList);

	CScene::CreateShaderResourceViews(pd3dDevice, UITexture, 15, false);

	CMaterial* UIMaterial = new CMaterial(1);
	UIMaterial->SetTexture(UITexture, 0);
	UIMaterial->SetShader(UIShader);

	SetMaterial(0, UIMaterial);
}

void UserInterface::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
}

void UserInterface::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
}

void UserInterface::ReleaseShaderVariables()
{
}

void UserInterface::SetLookAt(XMFLOAT3& target)
{
	XMFLOAT3 pos = Vector3::Add(target, Vector3::ScalarProduct(target, 1));
	pos.x += 40;
	pos.z += 20;
	pos.y += 90;
	SetPosition(pos);
...
void UserInterface::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
