...
cbuffer cbTimer_INFO : register(b9)
{
	float GameTimer : packoffset(c0);
};
...
float4 PSTextureUI_Timer(VS_TEXTURE_UI_OUTOUT input) : SV_TARGET
{
	return gtxtUITexture.Sample(gtxtUISampler, input.uv);
}
...
VS_TEXTURE_UI_OUTOUT VSTextureUI_BloodEffect(uint nVertexID : SV_VertexID)
{
	VS_TEXTURE_UI_OUTOUT output;
	float pos = 1.f;
	if (nVertexID == 0) { output.position = float4(-pos, +pos, 0.0f, 1.0f); output.uv = float2(0.f, 0.f); }
...
float4 PSTextureUI_BloodEffect(VS_TEXTURE_UI_OUTOUT input) : SV_TARGET
{
	return gtxtUITexture.Sample(gtxtUISampler, input.uv);
}
...
float4 PSTextureUI_ReadyFight(VS_TEXTURE_UI_OUTOUT input) : SV_TARGET
{
	return gtxtUITexture.Sample(gtxtUISampler, input.uv);
}

VS_TEXTURE_UI_OUTOUT VSTextureUI_LoseWin(uint nVertexID : SV_VertexID)
{
	VS_TEXTURE_UI_OUTOUT output;
	float pos = .3f;
...
float4 PSTextureUI_LoseWin(VS_TEXTURE_UI_OUTOUT input) : SV_TARGET
{
	return gtxtUITexture.Sample(gtxtUISampler, input.uv);
}
