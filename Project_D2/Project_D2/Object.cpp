...
void CGameObject::SetMesh(Mesh *pMesh)
{
	if (m_pMesh) m_pMesh->Release();
	m_pMesh = pMesh;
...
void CGameObject::Render(ID3D12GraphicsCommandList *pd3dCommandList, Camera *pCamera)
{
	OnPrepareRender();
...
{
	if (m_State != OBJECT_STATE::Moving) return;

	XMFLOAT3 xmf3Position = GetPosition();
	xmf3Position.x += m_xmf3Velocity.x * OBJECT_SPEED * fTimeElapsed;
	xmf3Position.y += m_xmf3Velocity.y * OBJECT_SPEED * fTimeElapsed;
	xmf3Position.z += m_xmf3Velocity.z * OBJECT_SPEED * fTimeElapsed;

	SetPosition(xmf3Position);
}

void CGameObject::SetTarget(XMFLOAT3 xmf3Target)
{
	XMFLOAT3 xmf3Look = GetLook();
	XMFLOAT3 xmf3Pos = GetPosition(); 
	XMFLOAT3 xmf3Dir = Vector3::Subtract(xmf3Target, xmf3Pos);
	m_xmf3Velocity = Vector3::Normalize(xmf3Dir);
}

void CGameObject::Attacked()
{
	if (m_State == OBJECT_STATE::Destroyed)  return;

	--m_nHp;
	if (m_nHp <= 0)
	{
		SetState(OBJECT_STATE::Destroyed);
		m_nHp = OBJECT_HP;
	}
}
...
CGameObject *CGameObject::LoadFrameHierarchyFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, CGameObject *pParent, FILE *pInFile, CShader *pShader)
{
	...
	for ( ; ; )
	{
		...
		else if (!strcmp(pstrToken, "<Mesh>:"))
		{
			...
			//pGameObject->m_BoundingBox = pMesh->GetBoundingBox();
			BoundingBox = pMesh->GetBoundingBox();
	...
	pGameObject->m_BoundingBox = BoundingBox;
	return(pGameObject);
}
...
CSkyBox::CSkyBox(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature) : CGameObject(1)
{
	...
	//pSkyBoxTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"SkyBox/Texture1.dds", RESOURCE_TEXTURE_CUBE, 0);
...
void CSkyBox::Render(ID3D12GraphicsCommandList* pd3dCommandList, Camera* pCamera)
{
...
Stadium::Stadium(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	StadiumMesh* mesh = new StadiumMesh(pd3dDevice, pd3dCommandList);
	SetMesh(mesh);
...
void Stadium::Render(ID3D12GraphicsCommandList* pd3dCommandList, Camera* pCamera)
{
	CGameObject::Render(pd3dCommandList, pCamera);
}
...
void Stadium::LoadMTLFile()
{

	std::string inputfile = "Arena.obj";
	tinyobj::ObjReaderConfig reader_config;
	reader_config.mtl_search_path = "./Texture"; // Path to material files
