...
void CAnimationSet::Animate(CGameObject* object)
{
	if (!Vector3::Compare(object->bones["Spine"]->GetPosition(), object->oldSpinePosition))
	{
		if (object->m_pSkinnedAnimationController->GetNowTrackAnimationSet(0) >= ANIMATION_JAB_KICK.first)
		{
			object->SetPosition({ object->bones["Spine"]->GetPosition().x, 100.f, object->bones["Spine"]->GetPosition().z });
		}
	}
	if ((object->isAlive) && (object->state != IDLE))
	{
		object->m_pSkinnedAnimationController->SetTrackAnimationSet(0, ANIMATION_COMBAT_MODE_A, ANIMATION_TYPE_LOOP);
		object->state = IDLE;
	}
	else
	{
	...
	object->m_pSkinnedAnimationController->m_pAnimationTracks[0].m_fPosition = 0.0f;
	m_fPosition = 0.0f;
}
...
void CAnimationSet::SetPosition(float fTrackPosition)
{
	...
	case ANIMATION_TYPE_ONCE:
	{
		...
		if (IsAnimate())// 애니메이션이 끝났을 때
		{
			if (isPlayer)
			{
				if (!Vector3::Compare(gScene->m_pPlayer->bones["Spine"]->GetPosition(), gScene->m_pPlayer->oldSpinePosition))
				{
					if (gScene->m_pPlayer->m_pSkinnedAnimationController->GetNowTrackAnimationSet(0) >= ANIMATION_JAB_KICK.first)
					{
				...
				{
					gScene->m_pPlayer->m_pSkinnedAnimationController->SetTrackAnimationSet(0, ANIMATION_COMBAT_MODE_A, ANIMATION_TYPE_LOOP);
			...
			{
				//Animate(gScene->m_ppHierarchicalGameObjects[0]);
				/*if (!Vector3::Compare(gScene->m_ppHierarchicalGameObjects[0]->bones["Spine"]->GetPosition(), gScene->m_ppHierarchicalGameObjects[0]->oldSpinePosition))
				{
					if (gScene->m_ppHierarchicalGameObjects[0]->m_pSkinnedAnimationController->GetNowTrackAnimationSet(0) >= ANIMATION_JAB_KICK)
					{
						gScene->m_ppHierarchicalGameObjects[0]->SetPosition({ gScene->m_ppHierarchicalGameObjects[0]->bones["Spine"]->GetPosition().x, 100.f, gScene->m_ppHierarchicalGameObjects[0]->bones["Spine"]->GetPosition().z });
					}
				}
				if ((gScene->m_ppHierarchicalGameObjects[0]->isAlive) && (gScene->m_ppHierarchicalGameObjects[0]->state != IDLE))
				{
					gScene->m_ppHierarchicalGameObjects[0]->m_pSkinnedAnimationController->SetTrackAnimationSet(0, ANIMATION_COMBAT_MODE_A, ANIMATION_TYPE_LOOP);
		...
		else// 애니메이션중
		{
			...
			if (isPlayer)
			{
				if (gScene->m_pPlayer->m_pSkinnedAnimationController->GetNowTrackAnimationSet(0) != ANIMATION_COMBAT_MODE_A.first)
				{
			...
			{
				if (gScene->m_ppHierarchicalGameObjects[0]->m_pSkinnedAnimationController->GetNowTrackAnimationSet(0) != ANIMATION_COMBAT_MODE_A.first)
				{
...
void CAnimationController::SetSpinePosition(CGameObject* object)
{
	object->oldSpinePosition = object->bones["Spine"]->GetPosition();
}

void CAnimationController::SetTrackAnimationSet(int nAnimationTrack, pair<UINT,UINT> nAnimationSet, UINT nType, bool bAnimFixed)
{
	if (m_pAnimationTracks)
	{
		if (m_pAnimationSets)
		{
			if (m_pAnimationSets->m_pAnimationSets[m_pAnimationTracks->m_nAnimationSet.first]->IsAnimate());

			// TODO : 애니메이션 타입 재정비 필요
			switch (m_pAnimationSets->m_pAnimationSets[m_pAnimationTracks->m_nAnimationSet.first]->m_nType)
			{
			case ANIMATION_TYPE_PINGPONG:
				break;
			case ANIMATION_TYPE_LOOP:
			...
			m_pAnimationSets->m_pAnimationSets[nAnimationSet.first]->m_nType = nType;

			if (gScene)
			{
				if (gScene->m_pPlayer)
					SetSpinePosition(gScene->m_pPlayer);

				//gScene->m_pPlayer->oldSpinePosition = gScene->m_pPlayer->bones["Spine"]->GetPosition();
				if (gScene->m_ppHierarchicalGameObjects[0])
					SetSpinePosition(gScene->m_ppHierarchicalGameObjects[0]);
			}
...
int CAnimationController::GetNowTrackAnimationSet(int nAnimationTrack)
{
	if (m_pAnimationTracks)
	{
		return m_pAnimationTracks[nAnimationTrack].m_nAnimationSet.first;
	}
	...
	if (m_pAnimationTracks)
	{
		...
		for (int j = 0; j < m_pAnimationSets->m_nAnimatedBoneFrames; j++)
		{
			...
			for (int k = 0; k < m_nAnimationTracks; k++)
			{
				...
				{
					CAnimationSet *pAnimationSet = m_pAnimationSets->m_pAnimationSets[m_pAnimationTracks[k].m_nAnimationSet.first];
					if (m_pAnimationTracks[k].m_nAnimationSet.first == ANIMATION_MOVE_FORWARD.first || m_pAnimationTracks[k].m_nAnimationSet.first == ANIMATION_MOVE_BACKWARD.first ||
						m_pAnimationTracks[k].m_nAnimationSet.first == ANIMATION_MOVE_LEFT.first || m_pAnimationTracks[k].m_nAnimationSet.first == ANIMATION_MOVE_RIGHT.first)
...
void CGameObject::SetTrackAnimationSet(int nAnimationTrack, pair<UINT, UINT> nAnimationSet)
{
...
CAngrybotObject::CAngrybotObject(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, CLoadedModelInfo *pModel, int nAnimationTracks)
{
	...
	m_pSkinnedAnimationController->SetTrackAnimationSet(0, ANIMATION_COMBAT_MODE_A, ANIMATION_TYPE_LOOP);
...
void Particle::Init(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* rootSignatrue)
{
	...
	if (objects.empty())
	{
		...
		for (int i = 0; i < 150; ++i)
		{
			...
			temp->m_pSkinnedAnimationController->SetTrackAnimationSet(0, make_pair(0,0));
