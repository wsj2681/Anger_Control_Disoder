...
void ResourceHelper::SetCbvSrvDescriptorIncrementSize(UINT size)
{
	this->gnCbvSrvDescriptorIncrementSize = size;
}

UINT ResourceHelper::GetCbvSrvDescriptorIncrementSize() const
{
	return this->gnCbvSrvDescriptorIncrementSize;
}

void ResourceHelper::SetRtvDescriptorIncrementSize(UINT size)
{
	this->gnRtvDescriptorIncrementSize = size;
}

UINT ResourceHelper::GetRtvDescriptorIncrementSize() const
{
	return this->gnRtvDescriptorIncrementSize;
}

void ResourceHelper::SetDsvDescriptorIncrementSize(UINT size)
{
	this->gnDsvDescriptorIncrementSize = size;
}

UINT ResourceHelper::SetDsvDescriptorIncrementSize() const
{
	return this->gnDsvDescriptorIncrementSize;
}

void ResourceHelper::SynchronizeResourceTransition(ID3D12GraphicsCommandList* pd3dCommandList, ID3D12Resource* pd3dResource, D3D12_RESOURCE_STATES d3dStateBefore, D3D12_RESOURCE_STATES d3dStateAfter)
{
...
ID3D12Resource* ResourceHelper::CreateTextureResourceFromDDSFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, wchar_t* pszFileName, ID3D12Resource** ppd3dUploadBuffer, D3D12_RESOURCE_STATES d3dResourceStates)
{
...
ID3D12Resource* ResourceHelper::CreateTextureResourceFromWICFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, wchar_t* pszFileName, ID3D12Resource** ppd3dUploadBuffer, D3D12_RESOURCE_STATES d3dResourceStates)
{
	ID3D12Resource* pd3dTexture = NULL;
...
ID3D12Resource* ResourceHelper::CreateTexture2DResource(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, UINT nWidth, UINT nHeight, UINT nElements, UINT nMipLevels, DXGI_FORMAT dxgiFormat, D3D12_RESOURCE_FLAGS d3dResourceFlags, D3D12_RESOURCE_STATES d3dResourceStates, D3D12_CLEAR_VALUE* pd3dClearValue)
{
	ID3D12Resource* pd3dTexture = NULL;
