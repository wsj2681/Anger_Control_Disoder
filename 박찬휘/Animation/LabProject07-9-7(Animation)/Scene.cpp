...
void Scene::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	boxer->m_pSkinnedAnimationController->SetTrackAnimationSet(nAnimTracks++, ANIMATION_IDLE);
	...
	cube->m_pSkinnedAnimationController->SetTrackAnimationSet(0, ANIMATION_IDLE);
	cube->isActive = false;
	...
	sphere->m_pSkinnedAnimationController->SetTrackAnimationSet(0, ANIMATION_IDLE);
...
bool Scene::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case VK_RETURN:
			break;
		case VK_F4:
			hierarchicalGameObjects.data()[OTHERPLAYER]->m_pSkinnedAnimationController->SetTrackAnimationSet(nAnimTracks++, ANIMATION_ATTACK_LOOP);
			break;
		case VK_F5:
			hierarchicalGameObjects.data()[OTHERPLAYER]->m_pSkinnedAnimationController->SetTrackAnimationSet(nAnimTracks++, ANIMATION_IDLE);
			break;
		case VK_F6:
			hierarchicalGameObjects.data()[OTHERPLAYER]->m_pSkinnedAnimationController->SetTrackAnimationSet(nAnimTracks++, ANIMATION_HOOK_L);
			break;
		case VK_INSERT:
